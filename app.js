...
function renderK6Players() {
    ...
    const tab = document.querySelector('.k6-premium-tab[data-k6="players"]');
    if (tab) {
        const countEl = tab.querySelector('.k6-premium-tab-count');
        if (countEl) countEl.textContent = `(${players.length}/16)`;
    }
    ...
    if (!players.length) {
        ...
        list.innerHTML = players.map((p, i) => {
            ...
            return `
            <div class="k6-player-item" id="k6pi-${i}" style="background:rgba(255,255,255,0.04);border-color:rgba(255,255,255,0.08);">
                <div class="k6-player-seed">${i+1}</div>
                <div class="k6-player-name" style="color:#fff;">
                ...
                <div class="k6-player-actions">
                    ${i > 0 ? `<button class="k6-player-move" onclick="k6MovePlayer(${i},-1)" title="Nasadiť vyššie" style="background:rgba(255,255,255,0.04);border-color:rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);">▲</button>` : '<span style="width:32px"></span>'}
                    ${i < players.length-1 ? `<button class="k6-player-move" onclick="k6MovePlayer(${i},1)" title="Nasadiť nižšie" style="background:rgba(255,255,255,0.04);border-color:rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);">▼</button>` : '<span style="width:32px"></span>'}
                    <button class="k6-player-del" onclick="k6RemovePlayer(${i})" title="Odstrániť" style="color:rgba(255,255,255,0.3);">×</button>
                </div>
            </div>`;
        }).join('');
    }
...
function k6StartTournament() {
    ...
    document.querySelectorAll('.k6-premium-tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.k6-panel').forEach(p => p.classList.remove('active'));
    document.querySelector('.k6-premium-tab[data-k6="bracket"]').classList.add('active');
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Kobe 6 tabs
    document.querySelectorAll('.k6-premium-tab').forEach(tab => tab.addEventListener('click', () => {
        document.querySelectorAll('.k6-premium-tab').forEach(t => t.classList.remove('active'));
        tab.classList.add('active');
        document.querySelectorAll('.k6-panel').forEach(p => p.classList.remove('active'));
